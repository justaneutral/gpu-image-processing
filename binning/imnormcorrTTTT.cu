...
#ifdef __sumsTT__
__global__ void corrTexTexSums_kernelTT(float *pCorr,size_t CorrPitch,int *pI,int *pISq,int *pIT,float cPixels,int wTemplate,int hTemplate,int w,int h)
{
    ...
    int SumTSq = 0;
    int SumIT = 0;
    for ( int y = 0; y < hTemplate; y+=step )
	{
        for ( int x = 0; x < wTemplate; x+=step )
		{
...
void corrTexTexSumsTT(float *dCorr,int CorrPitch,int *pI,int *pISq,int *pIT,int wTile,int wTemplate,int hTemplate,float cPixels,int sharedPitch,int w,int h,dim3 threads,dim3 blocks,int sharedMem)
{
    corrTexTexSums_kernelTT<<<blocks,threads>>>(dCorr,CorrPitch,pI,pISq,pIT,cPixels,wTemplate,hTemplate,w,h);
}
#endif
...
__global__ void corrTexTex_kernelTT(float *pCorr,size_t CorrPitch,float cPixels,size_t wTemplate,size_t hTemplate,int w,int h,int step)
{
    ...
    if(col>=w || row>=h || y>=hTemplate || x>=wTemplate)
        return;
    __shared__ int SumI;
    __shared__ int SumT;
...
CImg<float> mainncTT(CImg<unsigned  char> *idata,CImg<unsigned  char> *tdata,float& gpuTime,float& datatransfertime)
{
	...
	if(idata && tdata)
	{
		...
		///step = 16;
		threads.x =wTemplate; threads.y = hTemplate; threads.z = 1;
		blocks.x = wc; blocks.y = hc; blocks.z = 1;
